...
export const verifyOtp = async (req: Request, res: Response): Promise<unknown> => {
  try {
    ...
    const otpCheck = (await OtpInstance.findOne({
      where: { userId: id },
      order: [ [ 'createdAt', 'DESC' ]],
    })) as unknown as { [key: string]: string };

    if(!otpCheck || otpCheck.otp !== otp){
      return errorResponse(res, 'incorrect otp', 401)
    }

    let minutes = moment().diff(moment(otpCheck.createdAt), 'minutes');
    if(minutes > 5){
      return errorResponse(res, 'otp has expired', 401)
    }
    return successResponse(res, 'otp verifed', httpStatus.OK, {});
  } catch (error) {
    console.log(error);
    return serverError(res);
  }
